...
public class BoardService {
	...
	public Page<Board> findBoardList(Pageable pageable) {
		if (Flag.flag) {
			...
			pageSize = 10;
			sort = Sort.by("id").descending().and(Sort.by("title").ascending());
	...
	public Page<Board> findBoardListByUserName(Pageable pageable, String userName) {
		if (Flag.flag) {
			int pageNumber = pageable.getPageNumber() <= 0 ? 0 : pageable.getPageNumber() - 1;
			int pageSize = pageable.getPageSize();
			Sort sort = pageable.getSort();
		...
		log.info("KANG-20200803 >>>>> {} {}", CurrentInfo.get(), pageable);
		return this.boardRepository.findBoardListByUserName(pageable, userName);
	}
}
